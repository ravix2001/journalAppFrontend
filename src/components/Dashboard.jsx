...
function Dashboard() {
  ...
  const handleDeleteJournal = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/journal/journal/id/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdateProfile = async () => {
    try {
      await axios.put("http://localhost:8080/journal/user", profile, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      await axios.delete("http://localhost:8080/journal/user", {
        headers: { Authorization: `Bearer ${token}` },
      });
      handleLogout();
    } catch (err) {
